...
/*------------------------------------------------------------------*/
long long int n;
long long int a[lim], b[lim];
long long int dp[lim][2];
long long int fun(long long int idx, long long int p)
{
  if (dp[idx][p] != -1)
  {
    return dp[idx][p];
  ...
  if (p == 0)
  {
    x = max(fun(idx - 1, 0), a[idx] + fun(idx - 1, 1));
  }
  if (p == 1)
    x = max(fun(idx - 1, 1), b[idx] + fun(idx - 1, 0));
  return dp[idx][p] = x;
}
int main()
{
  ...
  for (long long int i = 1; i <= n; i++)
  {
    ans = max(ans, max(fun(i, 0), fun(i, 1)));
  }
  cout << ans;
